...
GR2_file::GR2_file(const char *path) : in(path, std::ios::in | std::ios::binary)
{
	...
	type_definition = (GR2_property_key*)(sections_data.data() + header.info.type_offset);	
}
...
void GR2_file::apply_marshalling(unsigned index)
{
	...
	in.read((char *)marshallings.data(), section.marshallings_count*sizeof(Marshalling));
...
{
	auto type_def = (GR2_property_key*)(sections_data.data() + section_offsets[m.target_section] + m.target_offset);
...
{	
	for (const auto &relocation : relocations[index]) {
		unsigned source_offset = section_offsets[index] + relocation.offset;
		unsigned target_offset = section_offsets[relocation.target_section] + relocation.target_offset;
		unsigned char *target_address = sections_data.data() + target_offset;
...
{
	std::vector<unsigned char> buffer(header.info.file_size - sizeof(Header));	
...
void GR2_file::read_relocations(Section_header &section)
{
	...
	if(section.relocations_count > 0)
		in.read((char *)relocations.back().data(), relocations.back().size()*sizeof(Relocation));
}
...
void GR2_file::read_section(unsigned index)
{
	...
	int ret = granny2dll.GrannyDecompressData(section.compression, 0,
		section.data_size,
		section_data.data(),
		section.first16bit,
		section.first8bit,
...
void GR2_file::read_section_headers()
{
	...
	in.read((char *)section_headers.data(), sizeof(Section_header)*header.info.sections_count);
	if (!in) {
		is_good = false;
...
void GR2_file::write(const char *filename)
{
	...
	header.info.crc32 = crc32c(0, (unsigned char *)sections.data(), sections.size()*sizeof(Section_header));
	for (unsigned i = 0; i < header.info.sections_count; ++i) {		
		header.info.crc32 = crc32c(header.info.crc32, (unsigned char *)relocations[i].data(), relocations[i].size() * sizeof(Relocation));
		if(section_offsets[i] < sections_data.size())
			header.info.crc32 = crc32c(header.info.crc32, (unsigned char *)&sections_data[section_offsets[i]], sections[i].data_size);
	}

	ofstream out(filename, std::ios::binary);
	out.write((char *)&header, sizeof(Header));	
	out.write((char *)sections.data(), sections.size() * sizeof(Section_header));

	for (unsigned i = 0; i < header.info.sections_count; ++i) {
		out.write((char *)relocations[i].data(), relocations[i].size() * sizeof(Relocation));
		if (section_offsets[i] < sections_data.size())
			out.write((char *)&sections_data[section_offsets[i]], sections[i].data_size);		
	}
}