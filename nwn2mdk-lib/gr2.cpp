...
static void compute_selectors(uint16_t selectors[4], uint16_t scale_offset_table_entries)
{
...
{		
	...
	for (int i = 0; i < knots_count; ++i) {
		encoded_knots_.push_back(data.knots_controls[i]);
		knots_.push_back(data.knots_controls[i] / data.one_over_knot_scale);
	}
	...
	for (int i = 0; i < controls_count; i += 3) {
		...
		encoded_controls_.emplace_back(a, b, c);				
		controls_.push_back(decode_D4nK16uC15u(a, b, c, scales, offsets));
	}
}
...
const std::vector<Vector3<uint16_t>>& GR2_D4nK16uC15u_view::encoded_controls() const
{
	return encoded_controls_;
}
...
GR2_D4nK8uC7u_view::GR2_D4nK8uC7u_view(GR2_curve_data_D4nK8uC7u& data)
{
	...
	for (int i = 0; i < controls_count; i += 3) {
		...
		uint8_t c = controls[i + 2];
		encoded_controls_.emplace_back(a, b, c);		
		controls_.emplace_back(decode_D4nK8uC7u(a, b, c, scales, offsets));
	}
}
...
const std::vector<Vector3<uint8_t>>& GR2_D4nK8uC7u_view::encoded_controls() const
{
	return encoded_controls_;
}
